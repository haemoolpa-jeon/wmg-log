...
import { flavorData, whiskyColors, Lang } from '@/lib/flavors'
import { countries } from '@/lib/countries'
import { storage, FlavorWithStrength, BottlingType, encodeReview } from '@/lib/storage'
import { domToPng } from 'modern-screenshot'
import { jsPDF } from 'jspdf'
import { ChevronLeft, RotateCcw, Globe, Sparkles, Share2, Copy, Check } from 'lucide-react'
import { correctText } from '@/lib/countries'
...
const ui = {
  ...
  // New
  bottlingType: { ko: '보틀링 타입', en: 'Bottling Type' },
  ...
  draftRestored: { ko: '임시저장된 내용을 불러왔습니다', en: 'Draft restored' },
...
const defaultWhisky: Whisky = { name: '', distillery: '', country: '', age: '', abv: '', cask: '', color: 0.7 }
const defaultScores = { nose: 20, palate: 20, finish: 20, balance: 20 }
const defaultNotes = { nose: '', palate: '', finish: '', overall: '' }
const defaultFlavors = { nose: [] as FlavorWithStrength[], palate: [] as FlavorWithStrength[], finish: [] as FlavorWithStrength[] }

export default function NewReviewPage() {
  const [lang, setLang] = useState<Lang>('ko')
  const [step, setStep] = useState<'form' | 'preview'>('form')
  const [reviewer, setReviewer] = useState('')
  const [whisky, setWhisky] = useState<Whisky>(defaultWhisky)
  ...
  // Load saved lang, reviewer, and draft
  useEffect(() => {
    ...
    if (draft) {
      if (draft.whisky) setWhisky({ ...defaultWhisky, ...draft.whisky })
      if (draft.scores) setScores({ ...defaultScores, ...draft.scores })
      if (draft.notes) setNotes({ ...defaultNotes, ...draft.notes })
      if (draft.flavors) setFlavors({ ...defaultFlavors, ...draft.flavors })
      if (draft.wouldRebuy) setWouldRebuy(draft.wouldRebuy)
      if (draft.reviewer) setReviewer(draft.reviewer)
    }
  }, [])

  // Auto-save draft
  const saveDraft = useCallback(() => {
    storage.saveDraft({ reviewer, whisky, scores, notes, flavors, wouldRebuy })
  }, [reviewer, whisky, scores, notes, flavors, wouldRebuy])
  ...
  const handleSave = () => {
    storage.saveReview({ reviewer, whisky, scores, notes, flavors, wouldRebuy })
    alert(ui.saved[lang])
  }

  const handleShare = async () => {
    const encoded = encodeReview({ reviewer, whisky, scores, notes, flavors, wouldRebuy })
  ...
  const reset = () => {
    setWhisky(defaultWhisky)
    setScores(defaultScores)
    setNotes(defaultNotes)
    setFlavors(defaultFlavors)
    setWouldRebuy(undefined)
    storage.clearDraft()
    setStep('form')
  }
    ...
    return (
      <div className="max-w-2xl mx-auto p-4">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <button onClick={handleExportPNG} disabled={exporting} className="py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-50">
            PNG
          </button>
          <button onClick={handleExportPDF} disabled={exporting} className="py-3 bg-red-600 text-white rounded-xl hover:bg-red-700 disabled:opacity-50">
            PDF
          </button>
        </div>
        <button onClick={handleShare} className="w-full mt-3 py-3 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 flex items-center justify-center gap-2">
          {showCopied ? <><Check size={18} /> {ui.copied[lang]}</> : <><Share2 size={18} /> {ui.shareLink[lang]}</>}
        </button>
        <button onClick={handleSave} className="w-full mt-3 py-3 bg-amber-600 text-white rounded-xl hover:bg-amber-700">
          {ui.saveLocal[lang]}
        </button>
      </div>
    )
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 pb-8">
      ...
      <section className="mb-5">
        ...
        <input type="text" placeholder={ui.reviewerPlaceholder[lang]} value={reviewer} onChange={e => setReviewer(e.target.value)}
      ...
      <section className="mb-5">
        ...
        <div className="space-y-2">
          <input type="text" placeholder={ui.whiskyName[lang] + ' *'} value={whisky.name} onChange={e => setWhisky(w => ({ ...w, name: e.target.value }))}
            className="w-full px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-amber-500" />
          <div className="grid grid-cols-2 gap-2">
            <input type="text" placeholder={ui.distillery[lang]} value={whisky.distillery} onChange={e => setWhisky(w => ({ ...w, distillery: e.target.value }))}
              className="px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-amber-500" />
            <select value={whisky.country} onChange={e => setWhisky(w => ({ ...w, country: e.target.value }))}
          ...
          <div className="grid grid-cols-3 gap-2">
            <input type="text" placeholder={ui.age[lang]} value={whisky.age} onChange={e => setWhisky(w => ({ ...w, age: e.target.value }))}
              className="px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-amber-500" />
            <input type="text" placeholder={ui.abv[lang]} value={whisky.abv} onChange={e => setWhisky(w => ({ ...w, abv: e.target.value }))}
              className="px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-amber-500" />
            <input type="text" placeholder={ui.cask[lang]} value={whisky.cask} onChange={e => setWhisky(w => ({ ...w, cask: e.target.value }))}
              className="px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-amber-500" />
          </div>
        </div>
      </section>
      ...
      <section className="mb-5 p-3 bg-gray-50 rounded-lg border border-dashed">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <input type="text" placeholder={ui.bottleNumber[lang] + ' (ex: 234/500)'} value={whisky.bottleNumber || ''} onChange={e => setWhisky(w => ({ ...w, bottleNumber: e.target.value }))}
              className="px-3 py-2 text-sm border rounded-lg" />
            <input type="text" placeholder={ui.price[lang]} value={whisky.price || ''} onChange={e => setWhisky(w => ({ ...w, price: e.target.value }))}
              className="px-3 py-2 text-sm border rounded-lg" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs text-gray-500">{ui.purchaseDate[lang]}</label>
              <input type="date" value={whisky.purchaseDate || ''} onChange={e => setWhisky(w => ({ ...w, purchaseDate: e.target.value }))}
                className="w-full px-3 py-2 text-sm border rounded-lg" />
            </div>
            <div>
              <label className="text-xs text-gray-500">{ui.openingDate[lang]}</label>
              <input type="date" value={whisky.openingDate || ''} onChange={e => setWhisky(w => ({ ...w, openingDate: e.target.value }))}
                className="w-full px-3 py-2 text-sm border rounded-lg" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {(['nose', 'palate', 'finish', 'balance'] as const).map(key => (
            <div key={key} className="bg-white p-3 rounded-lg border">
              ...
              <input type="range" min={0} max={25} value={scores[key]} onChange={e => setScores(s => ({ ...s, [key]: +e.target.value }))}
      ...
      {(['nose', 'palate', 'finish'] as const).map((key, i) => (
        <section key={key} className={`mb-4 p-3 rounded-lg border ${i === 0 ? 'bg-red-50/50 border-red-100' : i === 1 ? 'bg-amber-50/50 border-amber-100' : 'bg-orange-50/50 border-orange-100'}`}>
          ...
          <div className="relative mt-2">
            <textarea placeholder={ui[`${key}Note` as keyof typeof ui][lang]} value={notes[key]} onChange={e => setNotes(n => ({ ...n, [key]: e.target.value }))}
              rows={2} className="w-full px-3 py-2 pr-20 border rounded-lg focus:ring-2 focus:ring-amber-500 resize-none text-sm" />
            {notes[key].trim() && (
              <button onClick={() => handleCorrect(key)} disabled={correcting === key}
      ...
      <section className="mb-5 p-3 rounded-lg border bg-gray-50">
        ...
        <div className="relative">
          <textarea placeholder={ui.overallNote[lang]} value={notes.overall || ''} onChange={e => setNotes(n => ({ ...n, overall: e.target.value }))}
            rows={3} className="w-full px-3 py-2 pr-20 border rounded-lg focus:ring-2 focus:ring-amber-500 resize-none text-sm" />
          {notes.overall?.trim() && (
            <button onClick={() => handleCorrect('overall')} disabled={correcting === 'overall'}
              className="absolute right-2 top-2 flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 disabled:opacity-50">
      ...
      <button onClick={() => setStep('preview')} disabled={!whisky.name}
